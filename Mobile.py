...
client = commands.Bot(description="Bot prefix is MGC ", command_prefix=commands.when_mentioned_or("MGC" ), pm_help = True)



async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for MGChelp', type=2))
        ...
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='MGC'))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='watching over MGC'))
        await asyncio.sleep(5)
...
def is_iron(ctx):
    return ctx.message.author.id == "493373263674605578"
...
async def warn(ctx, userName: discord.User=None,*, message:str=None):
    if userName is None:
      await client.say('Please tag a person to warn user. Example- ``MGCwarn @user <reason>``')
      return
    else:
...
async def avatar(ctx, user: discord.Member=None):
    if user is None:
        ...
        embed.set_thumbnail(url='https://cdn.discordapp.com/avatars/538737881976406076/1901d8fb7c2dcf8dc7a8786592609cd5.webp?size=1024')
...
async def getuser(ctx, role: discord.Role = None):
    if role is None:
        await client.say('Please tag a role to get users having it. Example- ``MGCgetuser @role``')
        return
...
async def dm(ctx, user: discord.Member, *, msg: str):
   if user is None or msg is None:
       await client.say('Invalid args. Use this command like: ``MGCdm @user message``')
   if ctx.message.author.server_permissions.kick_members == False:
       await client.say('**You do not have permission to use this command**')
       return
   else:
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    if discord.utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        ...
        return
    if value is None:
        await client.say("Use this command like ``MGCrolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
    if ctx.message.author.server_permissions.manage_roles == False:
        await client.say('**You do not have permission to use this command**')
        return
...
async def announce(ctx, channel: discord.Channel=None, *, msg: str=None):
    ...
    if channel is None or msg is None:
        await client.say('Invalid args. Use this command like ``MGCannounce #channel text here``')
        return
    else:
...
async def mute(ctx, member: discord.Member=None, mutetime=None):
    ...
    if member is None:
        await client.say('Please specify member i.e. Mention a member to mute. Example-``MGCmute @user <time in minutes>``')
        return
    if mutetime is None:
        await client.say('Please specify time i.e. Mention a member to mute with time. Example-``MGCmute @user <time in minutes>``')
        return
...
async def unmute(ctx, member: discord.Member=None):
    if member is None:
      await client.say('Please specify member i.e. Mention a member to unmute. Example- ``MGCunmute @user``')
    if ctx.message.author.bot:
      return
    else:
      if ctx.message.author.server_permissions.kick_members == False:
...
async def access(ctx, member: discord.Member=None):
    if member is None:
      await client.say("Please specify a member to give access to him. Example- ``MGCaccess @user``")
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def setnick(ctx, user: discord.Member=None, *, nickname=None):
    if user is None:
      await client.say('Please tag a person to change nickname. Example- ``MGCsetnick @user <new nickname>``')
      return
    else:
